...
module.exports = {
  ...
  signUserAndSendOTP: async function (req, res) {
    try {
      ...
      const emailState = await authService.emailExistingAndOtpExistingCheck(email);
      ...
      if (emailState?.existingUser) {
        ...
        // existing but not verified → resend OTP (with resend throttle inside service)
        const resend = await authService.resendOtpForExistingUser(emailState.userId,email, req.ip);
        if (!resend.ok) {
          return rtnRes(res, resend.statusCode || 500, resend.message);
        }  
        return rtnRes(res, 200, 'Account found but not verified. OTP resent.');
      }

      // Create user with emailVerified = false
      const passwordHash = await authService.hashPassword(password);
      const storeUserResult = await authService.storeUser(email, name, passwordHash);
      if (!storeUserResult.isUserStored) return rtnRes(res, 500, 'Failed to register user');

      // Send OTP
      const otpResult = await authService.createAndSendOtp(storeUserResult.userId, email, req.ip);
  ...
  verifyOTP: async function (req, res) {
    try {
      ...
      if (!/^[0-9]{6}$/.test(otp)) return rtnRes(res, 400, 'Invalid OTP format');

      const verifyResult = await authService.verifyOtpForEmail(email, otp);
      if (!verifyResult.ok) {
        return rtnRes(res, verifyResult.statusCode || 400, verifyResult.message);
      }  
  ...
  login: async function (req, res) {
    try {
      ...
      if (!loginResult.ok){
        return rtnRes(res, loginResult.statusCode || 400, loginResult.message);
      }  

      return rtnRes(res, 200, 'Login successful', { token: loginResult.token });
    } catch (err) {
