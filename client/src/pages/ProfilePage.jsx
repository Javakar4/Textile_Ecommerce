...
const ProfilePage = () => {
    const { userData, addresses, setAddresses, setUser, setShowUserLogin } = UseAppContext();
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                        ...
                        {showAddAddress && (
                            <form onSubmit={handleAddAddress} className="bg-gradient-to-br from-amber-50/50 to-white p-5 rounded-xl mb-6 border-2 border-amber-700 shadow-sm animate-in fade-in slide-in-from-top-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    ...
                                    <input required placeholder="ZIP Code" className="border-2 border-gray-200 p-2.5 rounded-lg focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-200 transition-all" value={newAddress.zip} onChange={e => setNewAddress({...newAddress, zip: e.target.value})} />
                                ...
                                <div className="flex gap-2">
                                    <button type="submit" className="bg-amber-700 text-white px-6 py-2 rounded-lg hover:bg-amber-800 transition-all duration-200 font-medium shadow-sm hover:shadow-md">Save Address</button>
                        ...
                        <div className="space-y-4">
                            {addresses.length === 0 ? (
                                ...
                                addresses.map((addr) => (
                                    <div key={addr.id} className={`border rounded-lg p-4 relative transition-all ${addr.isDefault ? 'border-amber-700 bg-amber-50' : 'border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}>
                                        {addr.isDefault && (
                                        ...
                                        <div className="absolute bottom-4 right-4 flex gap-2">
                                            {!addr.isDefault && (
                                                <button onClick={() => setDefaultAddress(addr.id)} className="text-sm text-amber-600 hover:text-amber-700 font-medium hover:underline">
                                                    Set Default
                                                </button>
                                            )}
                                            <button onClick={() => removeAddress(addr.id)} className="text-red-400 hover:text-red-600 ml-2" title="Remove Address">
                                                <FaTrash />
                                            </button>
                                        </div>
                                    </div>
